                      ...
                      <div className="flex gap-2">
                        {["35", "42", "55"].map((v) => (
                          <Button
                            key={v}
                            type="button"
                            variant={data.vorlauftemperatur === v ? "default" : "outline"}
                            size="sm"
                            onClick={() => update("vorlauftemperatur", v)}
                          >
                            {v}°C
                          </Button>
                        ))}
                        <Input
                          ...
                          type="number"
                          placeholder="Andere"
                          value={!["35", "42", "55"].includes(data.vorlauftemperatur) ? data.vorlauftemperatur : ""}
                          onChange={(e) => update("vorlauftemperatur", e.target.value)}
                          className="w-24"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <RadioGroup value={data.wpHeizkoerper} onValueChange={(v) => update("wpHeizkoerper", v)} className="mt-2 flex gap-4">
                        <div className="flex items-center gap-2">
                          <RadioGroupItem value="ja" id="wphk-ja" />
                          <Label htmlFor="wphk-ja" className="font-normal">Ja</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <RadioGroupItem value="nein" id="wphk-nein" />
                          <Label htmlFor="wphk-nein" className="font-normal">Nein</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <RadioGroupItem value="unbekannt" id="wphk-unbekannt" />
                          <Label htmlFor="wphk-unbekannt" className="font-normal">Unbekannt</Label>
                        </div>
                      </RadioGroup>
                    </div>
                  </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-5">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 mb-1.5">
                        Zählen Sie alle täglichen Dusch- und Badevorgänge im gesamten Haushalt
                      </p>
                      <div className="flex gap-2 items-center">
                        {["1", "2", "3", "4", "5"].map((v) => (
                          <Button
                            key={v}
                            type="button"
                            variant={data.duschenProTag === v ? "default" : "outline"}
                            size="sm"
                            onClick={() => update("duschenProTag", v)}
                            className="w-10"
                          >
                            {v}
                          </Button>
                        ...
                        <Input
                          ...
                          type="number"
                          placeholder="6+"
                          value={Number(data.duschenProTag) > 5 ? data.duschenProTag : ""}
                          onChange={(e) => update("duschenProTag", e.target.value)}
                          className="w-20"
                        />
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-5">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 mb-1.5">
                        Die gewünschte Durchschnittstemperatur in Ihren Wohnräumen
                      </p>
                      <div className="flex gap-2">
                        {["20", "21", "22", "23"].map((v) => (
                          <Button
                            key={v}
                            type="button"
                            variant={data.raumtemperatur === v ? "default" : "outline"}
                            size="sm"
                            onClick={() => update("raumtemperatur", v)}
                          >
                            {v}°C
                          </Button>
                        ))}
                        <Input
                          ...
                          type="number"
                          placeholder="Andere"
                          value={!["20", "21", "22", "23"].includes(data.raumtemperatur) ? data.raumtemperatur : ""}
                          onChange={(e) => update("raumtemperatur", e.target.value)}
                          className="w-24"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <RadioGroup value={data.automatischeRaumregler} onValueChange={(v) => update("automatischeRaumregler", v)} className="mt-2 flex gap-4">
                        <div className="flex items-center gap-2">
                          <RadioGroupItem value="ja" id="arr-ja" />
                          <Label htmlFor="arr-ja" className="font-normal">Ja</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <RadioGroupItem value="nein" id="arr-nein" />
                          <Label htmlFor="arr-nein" className="font-normal">Nein</Label>
                        </div>
                      </RadioGroup>
                    </div>
                  </div>
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-border">
            ...
            {step < TOTAL_STEPS - 1 ? (
              ...
              <Button variant="hero" onClick={handleSubmit}>
                Verfeinerte Ergebnisse berechnen <ArrowRight className="ml-1 h-4 w-4" />
              </Button>
            )}
          </div>
        </div>
